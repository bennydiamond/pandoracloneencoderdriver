...
static int pandora_init (int fd,
		    __attribute__ ((unused)) unsigned long *id,
		    __attribute__ ((unused)) unsigned long *extra) 
{
    unsigned char cmd[16] = { 0xFE, 0xFE, 0x0F, 0x7A, 0x39, 0xB1, 0xD7, 0xAB, 0xBF, 0xD8, 0x6B, 0xDA, 0xC1, 0xFE, 0xFE, 0x0F };
	unsigned char expected_resp[14] = {0xFF, 0xAF, 0x63, 0x18, 0x6A, 0x7C, 0x0C, 0xBB, 0xC5, 0x2D, 0x39, 0x5E, 0xFF, 0xFF };
	unsigned char resp[14];
	...
	for (unsigned char i = 0; i < 14; i++)
		if (readchar(fd, resp + i, 100))
			return -1;

	if (memcmp(resp, expected_resp, sizeof(expected_resp)))
		return -1;
...
{ "--pandora",	"-pand",	"Pandora arcade serial encoder",
	B115200, CS8,
	PANDORA_CLONE, 0x00, 0x00, 1, pandora_init },
...
int main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		...
		if (!strcasecmp(argv[i], "--help")) {
			...
			daemon_mode = 1;
		} else if (!strcasecmp(argv[i], "--noldisc")) {
			no_ldisc = 1;
	...
	if (ioctl(fd, SPIOCSTYPE, &devt) < 0) {
		fprintf(stderr, "inputattach: can't set device type  0x%lx:0x%lx:0x%lx\n", type[i]->type, id, extra);
		return EXIT_FAILURE;
	}
